...
impl StatusItem {
    ...
    /// Updates the title of a root menu item without replacing the menu.
    pub fn set_menu_item_title(&mut self, index: usize, title: impl AsRef<str>) -> bool {
        self.menu.set_item_title(index, title)
    }
    ...
    /// Sets the status button image from encoded image data, such as SVG or PNG.
    pub fn set_image_data(&mut self, image_data: &[u8]) -> bool {
        unsafe {
            let data = NSData::dataWithBytes_length(
                image_data.as_ptr() as *mut c_void,
                image_data.len(),
            );
            ...
            let Some(button) = self.inner.button() else {
                return false;
            };
...
impl Menu {
    ...
    fn set_item_title(&mut self, index: usize, title: impl AsRef<str>) -> bool {
...
impl MenuItem {
    /// Creates a native macOS menu separator.
    pub fn separator() -> Self {
        unsafe {
            Self {
                inner: NSMenuItem::separatorItem(),
                title: String::new(),
                callback: None,
                submenu: None,
            }
    ...
    fn set_title(&mut self, title: impl AsRef<str>) {
        let title = title.as_ref();
        unsafe {
            self.inner.setTitle(&NSString::from_str(title));
        }
